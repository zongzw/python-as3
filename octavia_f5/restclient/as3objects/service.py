...
def get_service(loadbalancer, listener):
    ...
    if listener.default_pool:
        poolid = listener.default_pool.id
        pools = list(filter(lambda x: x.id == poolid, loadbalancer.pools))
        if len(pools) == 0: 
            raise exceptions.SingleCreateDetailsMissing()
        pool = pools[0]
        if pool.provisioning_status != lib_consts.PENDING_DELETE:
            default_pool = m_pool.get_name(listener.default_pool.id)
    ...
    # session persistence
    if listener.default_pool and pool.session_persistence:
